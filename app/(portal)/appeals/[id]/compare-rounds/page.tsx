...
import { RoundCompareView } from "@/components/round-compare-view"
    ...
    <div className="w-full">
      ...
      <div className="mt-4 flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Side-by-side comparison of AI summaries, decision trees, and
            evidence across appeal cycles.
          </p>
        </div>
        <Link
          href={`/appeals/${appeal.id}/timeline`}
      ...
      {appeal.cycles.length >= 2 && (
        <p className="mt-4 flex items-center gap-2 text-sm text-muted-foreground">
          ...
          Comparing {appeal.cycles.length} appeal rounds for {appeal.caseId}
        </p>
      )}
