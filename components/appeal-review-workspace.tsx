      ...
      <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-[1fr_22rem]">
        ...
        <div className="flex flex-col gap-6">
          ...
          {isActiveCycle ? (
                    ...
                    {decisionActions.map((action) => (
                      <Button
                        ...
                        className={cn(
                          ...
                          getOutcomeButtonClass(action.type),
                        )}
                        onClick={() => requestAction(action.type)}
                      >
                        <DecisionActionIcon type={action.type} />
                        {action.label}
                      </Button>
                    ))}
                  </div>
              ...
              {reAppealAction && (
                <div className="mt-4 border-t border-border pt-4">
                  ...
                  <Button
                    ...
                    onClick={() => requestAction(reAppealAction.type)}
                  >
                    {reAppealAction.label}
                  </Button>
                  <p className="mt-1.5 text-xs text-muted-foreground">
                    Opens a new appeal round with a fresh decision tree and AI
                    summary.
                  </p>
                </div>
              )}
            ...
            <section className="rounded-xl border border-border bg-muted/30 p-5 shadow-sm">
              <p className="text-sm text-muted-foreground">
                Viewing a prior round. Workflow actions apply to the active cycle
                only.
              </p>
              <Link
          ...
          {state.cycles.length >= 2 && (
            <Link
              href={`/appeals/${appeal.id}/compare-rounds`}
